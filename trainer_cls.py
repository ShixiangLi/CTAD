...
from utils.diy_loss import ClusterAwareLoss
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.cls_criterion = ClusterAwareLoss(
            n_clusters=3,  # 可以根据数据特性调整
            device=self.args.device,
            feature_levels=self.args.feature_levels
        )
    ...
    def train(self):
        ...
        if not self.cls_criterion.load_cluster_models():
            self.cls_criterion.collect_features(
                self.encoder, 
                self.train_loader, 
                no_avg=self.args.no_avg
            )
            self.cls_criterion.initialize_clusters()
        ...
        for epoch in range(self.start_epoch, self.args.num_epochs):
            ...
            self.cls_criterion.reset_counts()
            ...
            for i, (images, _, _, _, _) in enumerate(self.train_loader):
                ...
                for fl in range(self.args.feature_levels):
                    ...
                    cluster_ids = self.cls_criterion.get_cluster_id(input, fl)
                    ...
                    loss = self.cls_criterion.calculate_loss(output, input, cluster_ids)
                    ...
                    # 每50次迭代更新簇权重
                    if i % 30 == 0:
                        self.cls_criterion.update_weights()
            ...
            print("current best pix_f1: ", best_pix_f1)
    ...
    def test(self, vis=False, checkpoint_path=None):
        ...
        if checkpoint_path is not None:
            checkpoint = torch.load(os.path.join(checkpoint_path, self.args.save_prefix, 'best-pix.pth'))
            state_dict = checkpoint['state_dict']
            for i, model in enumerate(self.models):
                model.load_state_dict(state_dict[i])
        for model in self.models:
    ...
    def test_separate(self, class_name, vis=False):
        ...
        if vis:
            ...
            visulizer.plot(test_imgs, pred_label, gt_mask, file_names, img_anomalies, decode_list)
