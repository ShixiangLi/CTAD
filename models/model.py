...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        # Adding 1D convolutions for ref_mca processing
        h = w = int(math.sqrt(seq_len))
        # Height direction 1D convolution (applies across each column)
        ...
        # Width direction 1D convolution (applies across each row)
    ...
    def process_ref_mca(self, ref_mca):
        # Process ref_mca with 1D convolutions in height and width directions
        batch_size, seq_len, d_model = ref_mca.shape
        
        # Reshape for height direction convolution (treat width as batch dimension)
        # [B, L, D] -> [B*W, H, D]
        height_input = ref_mca.reshape(batch_size * self.w, self.h, d_model)
        height_input = height_input.permute(0, 2, 1)  # [B*W, D, H]
        height_output = self.conv1(height_input)  # [B*W, D, H]
        height_output = height_output.permute(0, 2, 1)  # [B*W, H, D]
        height_output = height_output.reshape(batch_size, seq_len, d_model)
        
        # Reshape for width direction convolution (treat height as batch dimension)
        # [B, L, D] -> [B*H, W, D]
        width_input = ref_mca.reshape(batch_size * self.h, self.w, d_model)
        width_input = width_input.permute(0, 2, 1)  # [B*H, D, W]
        width_output = self.conv2(width_input)  # [B*H, D, W]
        width_output = width_output.permute(0, 2, 1)  # [B*H, W, D]
        width_output = width_output.reshape(batch_size, seq_len, d_model)
        
        # Combine the two convolution outputs
        processed_ref_mca = height_output + width_output
        
        return processed_ref_mca

    def forward(self, x, ref_mca, gate):
        # Process ref_mca with 1D convolutions before using it in attention mechanisms
        if ref_mca is not None:
            processed_ref_mca = self.process_ref_mca(ref_mca)
        else:
            processed_ref_mca = None
        ...
        if self.block_mode == 'serial':
            ...
            if self.mca is not None:
                ...
                for ref_split in processed_ref_mca.split(self.seq_len, dim=1): # refl x (N, L, D)
                    mca_x = mca_x + self.mca(
                        x, ref_split, ref_split, attn_mask=mask
                    )[0]
            ...
            if self.ca is not None:
                ca_x = self.ca(
                    x, processed_ref_mca, processed_ref_mca
                )[0]
            ...
            if self.nsa is not None:
                mask = self.generate_mask()
                nsa_x = self.nsa(
                    x, processed_ref_mca, x, attn_mask=mask
                )[0]
